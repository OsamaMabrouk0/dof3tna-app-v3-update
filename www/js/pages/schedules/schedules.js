(function (App) {
    ...
    /* ══════════════════════════════════════════════
       ✅  CONFIG — الفرقة الثانية فقط
       أضف / احذف صور من كل images array حسب الحاجة
       ══════════════════════════════════════════════ */
    const TYPES = {
        intizam: {
            key: 'intizam',
    ...
    let activeImgIdx = 0;  // index within TYPES[activeType].images
    let zoom = 1;
    let pan = { x: 0, y: 0 };
    let dragOrigin = null;
...
<div id="sp-root" class="container mx-auto max-w-4xl pb-24 space-y-4">
    ...
    function renderViewerContent() {
        ...
        <div class="sp-top-acts">
            ...
            <button class="sp-act" id="sp-download" title="تحميل الصورة">
                <i class="fa-solid fa-download"></i>
            </button>
            <button class="sp-act" id="sp-share" title="مشاركة">
        ...
        <div class="sp-toolbar">
            ...
            <span class="sp-tb-sep"></span>
            <button class="sp-tb-btn" id="sp-zoom-fit" title="ملاءمة للشاشة"><i class="fa-solid fa-compress"></i></button>
        </div>`;
    }
    ...
    function attachViewerEvents() {
        ...
        on(document.getElementById('sp-zoom-fit'), 'click', () => { zoom = 1; pan = { x: 0, y: 0 }; applyTransform(true); updateZoomLabel(); });
        ...
        /* Download */
        on(document.getElementById('sp-download'), 'click', () => {
            const a = document.createElement('a');
            a.href = img.src; a.download = currentImage().id + '.jpg'; a.click();
        });

        /* Share */
        on(document.getElementById('sp-share'), 'click', () => {
            const t = TYPES[activeType];
            const cur = currentImage();
            if (navigator.share) {
                navigator.share({ title: `جدول ${t.label} — ${cur.label}`, url: cur.src }).catch(() => { });
            } else if (navigator.clipboard) {
                navigator.clipboard.writeText(cur.src).then(() => {
                    App.Toast?.success('تم نسخ رابط الصورة', 'تم النسخ');
                });
            }
        });
    }
    ...
    function openLightbox() {
        ...
        const closeLb = () => { lb.style.opacity = '0'; setTimeout(() => lb.remove(), 200); };
    ...
    function attachViewerEvents() {
        ...
        on(document.getElementById('sp-download'), 'click', () => {
            const url = img.src;
            const a = document.createElement('a');
            a.href = url; a.download = currentImage().id + '.jpg';
            a.click();
        });
        ...
        on(document.getElementById('sp-share'), 'click', () => {
            ...
            const cur = currentImage();
            if (navigator.share) {
                navigator.share({ title: `جدول ${t.label} — ${cur.label}`, url: cur.src })
                    .catch(() => { });
            } else if (navigator.clipboard) {
                navigator.clipboard.writeText(cur.src).then(() => {
                    App.Toast?.success('تم نسخ رابط الصورة', 'تم النسخ');
