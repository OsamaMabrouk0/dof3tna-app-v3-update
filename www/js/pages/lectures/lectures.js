(function(App) {
    ...
    App.Pages.lectures = function() {
        ...
        function renderItems() {
            ...
            if (searchQuery.trim() && data.length > 0) {
                html = `<div class="flex items-center gap-2 mb-3 px-1">
                            ...
                            <span class="text-xs font-bold text-blue-500">«${searchQuery}»</span>
        ...
        // ── back button (آمن — يتحقق من السايدبار أولاً) ──────────────
        function setupBackButtonSupport() {
            const popstateHandler = function(e) {
                // ─ الأولوية 1: إذا كان السايدبار مفتوحاً على الموبايل
                //   نتركه لـ ui.js ولا نتدخل
                const sidebar = document.getElementById('sidebar');
                const sidebarOpen = sidebar &&
                    window.innerWidth < 768 &&
                    !sidebar.classList.contains('translate-x-full');

                if (sidebarOpen) return; // ui.js يتولى الأمر

                // ─ الأولوية 2: إذا كنا داخل مسار → ارجع مستوى واحد
                ...
                // إذا كنا في الجذر → لا نفعل شيئاً (يسمح للـ router بالتصرف)
            };
