(function(App) {
    ...
    function _renderBadge() {
        ...
        notifBtn.querySelectorAll('.notif-badge').forEach(el => el.remove());

        const unreadCount = _cachedNotifications.filter(n => !n.isRead).length;
        if (unreadCount > 0) {
            const badge = document.createElement('span');
            badge.className = 'notif-badge absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 ' +
                'bg-red-500 rounded-full flex items-center justify-center ' +
                'text-white text-[9px] font-bold border-2 border-[var(--bg-main)] ' +
